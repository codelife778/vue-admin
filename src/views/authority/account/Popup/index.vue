<template>
  <div>
    <el-dialog title="编辑用户" :visible.sync="dialogVisible" width="30%" @close="handleClose">
      <el-form ref="form" :rules="rules" :model="form" label-width="80px" :inline="false" size="normal">
        <el-form-item label="用户名" prop="username">
          <el-input v-model="form.username" :readonly="form.id && form.id.length>0" size="small" />
        </el-form-item>
        ...
        <el-form-item label="用户头像">
          <el-upload
            class="avatar-uploader" action="http://localhost/lejuAdmin/material/uploadFileOss" :headers="headers"
...
export default {
    ...
    methods: {
        ...
        beforeAvatarUpload(file) {
            var reg = /image\/(jpeg|png|gif|webp)/gi
        ...
        onSubmit() {
            this.$refs.form.validate((valid) => {
                if (valid) {
                    var api = null
                    if (this.form.id) {
                        api = accountApi.updateUserRoles
                    } else {
                        api = accountApi.saveUserRoles
                    }
